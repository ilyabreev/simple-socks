			...
			binary
				...
				.tap(function (args) {
					if (args.cmd === RFC_1928_COMMANDS.CONNECT) {
						if (self.options.allowFrom.indexOf(socket.remoteAddress.replace("::ffff:", "")) == -1) {
                            return end(RFC_1928_REPLIES.CONNECTION_REFUSED, args);
                        }


                        var destination = net.createConnection({ port: args.dst.port, host: args.dst.addr, localAddress: socket.localAddress.replace("::ffff:", "") },
							args.dst.port,
							args.dst.addr,
							function () {
